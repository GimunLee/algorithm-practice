...
public class 삼성문제복기_1번_버스2_Memo {
	private static int cnt;
	private static int[][] m;
	private static int N;
	private static int[][][] memo; // 계산한 경로 개수를 저장할 변수, 메모이제이션
...
//			}
			...
			memo = new int[N][N][3]; // 몇행 몇열의 모드가 뭐일경우의 경우의 수를 저장
			for (int i = 0; i < N; i++) {
				for (int j = 0; j < N; j++) {
					for (int k = 0; k < 3; k++) {
						memo[i][j][k] = -1; // 계산 안한것을 표시하기 위해서 -1로 초기화
					}
				}
			}
	 ...
	 * r,c위치의 꼬리의 mode 상태에서 도착지점까지 갈 수 있는 경로의 개수 리턴
	 */
	public static int dfs(int r, int c, int mode) {
		// 이미 계산한 상태값이라면, 재귀호출하지않고 저장해 둔 값을 리턴
		if (memo[r][c][mode] != -1) {
		...
		if (mode == 0) {// 가로
			...
			if (m[r + 1][c + 2] == 0) {
				cnt += dfs(r + 1, c + 1, 0); // 가로
			}
			if (m[r + 1][c + 2] == 0 && m[r + 2][c + 1] == 0 && m[r + 2][c + 2] == 0) {
				cnt += dfs(r + 1, c + 1, 1); // 대각
			}
			if (m[r + 2][c + 1] == 0) {
				cnt += dfs(r + 1, c + 1, 2); // 세로
			}
		} else {// 세로

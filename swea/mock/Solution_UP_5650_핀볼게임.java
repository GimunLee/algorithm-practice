...
public class Solution_UP_5650_핀볼게임 {
				...
				dir: for (int i = 0; i < dr.length; i++) { // 4방향으로 보내봄
					...
					here: while (true) {
						...
						if (map[nR][nC] >= 1 && map[nR][nC] <= 5) { // 블럭이라면
							next_dir = get_block(map[nR][nC], next_dir);
							r = nR;
							c = nC;
							cnt++;
							continue here;
	...
	/** 블록 r,c 좌표, 진행중인 방향 받아서, 방향 리턴, 벽이면 블럭 5와 동일 */
	private static int get_block(int type, int dir) {
		return block_dir[type - 1][dir];
	} // end of get_block()
